...
class CommentManagerPDO extends CommentManager
{
    ...
    public function getList($start = -1, $limit = -1, $status = null, $postId = null, $authorId = null)
    {
        $sql = "SELECT c.*, post.title AS 'post',
                ...
                FROM comment AS c 
                LEFT JOIN user AS author ON c.user_id = author.id 
                LEFT JOIN post AS post ON c.post_id = post.id ";
    ...
    {
        $request = $this->dao->prepare(
            'INSERT INTO comment SET post_id = :post_id,
                                parent_id = :parent_id,
                                user_id = :user_id,
                                message = :message,
                                date_add = :date_add,
    ...
    {
        $request = $this->dao->prepare(
            'UPDATE comment SET post_id = :post_id,
                                user_id = :user_id,
                                message = :message,
                                date_add = :date_add,
                                date_edit = :date_edit,
